...
## v0.4.32 continuation: device recovery and SDK distribution

### Device-recovery seek refusal

A pending AudioUnit StreamFormat notification closes render admission and changes the output to Stopping. Normal session drain reconciles an unchanged device rate, but an in-flight seek owns the Flush lifecycle, which excludes that normal recovery path. Flush stops and resets the generation, then output activation formerly rejected the still-pending device invalidation as Invalid. The dispatcher maps that to Flush refusal. This reproduces the failure class retained in `proofs/defect/original-6-mkv`; the old receipt does not contain a stack identifying its exact interleaving.

`NativeAudioOutput::activate` now reconciles pending device invalidation on the serialized owner before activating a generation. A notification racing that check yields Quiescing for another bounded lifecycle pass. Actual rate changes retain the existing fail-closed rate validation. No time conversion or callback/presentation scheduling changes are involved.

The new `wamkit_device_recovery_test.mm` reuses the frozen audio fixture by inclusion, without editing it, and invokes its installed fake AudioUnit property listener. Thirty baseline iterations failed; thirty fixed iterations passed. The temporary-revert script removes reconciliation, observes failure, restores exact source bytes, and observes success: [receipt](proofs/continuation/device-revert.json). The identity-gated host seam publishes the same listener notification at seek activation without modifying any system audio property.

The measured host negative control rebuilt the exact HEAD activation body while preserving only the fake notification seam: it failed with a native refusal and missing exact seek completion, then passed after restoration and rebuild ([receipt](proofs/continuation/device-host-revert.json)).

All eight `wamkit_host_*` tests passed **30 consecutively each**: MP4, Matroska, named refusal, API, quarantine, replacement, device recovery, and packaging. The [first campaign](proofs/continuation/host-30.log) retained one MP4 failure after 18 passes: preparation took 2.65 seconds, seek began at 3.76 seconds, and the existing four-second harness close cancelled it. A sibling GUI proof was observed running. The [MP4 retry](proofs/continuation/host-mp4-30-isolated.log), begun after no sibling WAM GUI process remained, passed 30/30 without acceptance changes. Full streams and process identities are indexed in [host receipts](proofs/continuation/host-receipts.json).

### Swift Package and consumer

The root [Package.swift](../../Package.swift) exposes a local binary target at `build/WAMKit.xcframework`. [Assembly and build](../../scripts/build_wamkit_swift.py) use the built framework and local SwiftPM inputs, arm64 only, with the honest macOS 26 floor. The [SwiftUI sample](../../examples/WAMKitSwiftHost/WAMKitSwiftHost.swift) imports WAMKit, embeds its view via NSViewRepresentable, observes state, and drives all five transport actions with exact rational seek input.

The package build and all three embedding checks passed **4/4** ([log](proofs/continuation/swift-third.log)). MP4 and Matroska prove drawn frames, exact first PTS zero, `T=1001/30000`, `A=267/8000`, clock `1.0000`, and clean uncharged close. The refusal test preserves `HeAacSbrDecoderDelayUnproven` and its related native refusal. [All Swift receipts](proofs/continuation/swift-receipts.json), [XCFramework metadata](proofs/continuation/xcframework.json), and [five-image/22-symbol closure audit](proofs/continuation/swift-framework-audit.json) are retained. The first compile caught a Swift imported-enum type mismatch. Initial automatic SwiftUI scene opening produced no window/telemetry under background launch; explicit AppKit window ownership fixed the sample's startup. Failed receipts remain visible.

### Signing preparation

[Identity enumeration](proofs/continuation/signing-identities.log) found **zero valid identities**. The [script](../../scripts/sign_release.sh) passed its no-secret no-op and missing-identity gate, then signed build-tree copies ad hoc with hardened runtime. Every top-level bundle passed `codesign --verify --deep --strict`. All three app assessments returned Gatekeeper rejection (rc=3), as expected for ad-hoc signatures; this is not a Developer ID acceptance claim. The [signing report](proofs/continuation/signing-report.json) records the nested code order, unchanged entitlement policy and archive hash. The refreshed prepared ZIP is retained under `build/wamkit-signing-final/` and was not submitted; its [final receipt](proofs/continuation/signing-final-report.json) and [signature metadata](proofs/continuation/signing-metadata.json) bind the current artifacts.

No hardened-runtime exception was added. Quick Look retains its existing app-sandbox entitlement; WAM, WAMKit and both hosts receive none. Automatic approval review rejected a proposed allow-jit exception for lack of demonstrated necessity; it was removed before any write or signing execution. The owner must obtain a Developer ID Application certificate and matching private key. Trusted timestamping and notarization/stapling require a later authorized online release pass; this offline script deliberately does neither. CI skips without its identity secret and otherwise signs a copy of the deployed app.

### Dogfooding boundary

The audit found direct Qt access to the native session pointer for metrics, seek ceiling and observation draining, redundant session/preview includes, and an unused direct session-construction function. These are removed. Session and observation-bridge ownership are now private in the common owner; both WAMKit and Qt consume the same owner accessors. Host policy crosses `NativeEmbeddingSupport`, while Qt presentation construction resides in `src/platform/macos/qt_native_presentation_adapter.mm`.

The [guard](../../tests/wamkit_dogfooding_test.py) rejects direct or relative includes into unexposed macOS internals and native session/backend access tokens. Its explicit boundaries are the shared native owner, preflight and host-policy headers, plus the Qt scenegraph presentation item in its app adapter. This is source-layer enforcement, not a claim that the Qt app loads the dynamic framework. The [HEAD baseline audit](proofs/continuation/dogfooding-baseline.log) fails; [negative controls](proofs/continuation/dogfooding-negative.json) reject internal includes and direct session access while accepting the owner boundary. The refactored tree passes.


The hardened ad-hoc sample copies were additionally launched through the measured checker. Both failed before GUI startup: dyld rejected WAMKit with “mapping process and mapped file (non-platform) have different Team IDs” ([receipts and loader logs](proofs/continuation/signed-hosts/)). This is an explicit signing-identity validation blocker, not a passed embedding proof. Library validation remains enabled; same-team Developer ID execution must be proved by the owner once the certificate and private key are available. The normal build hosts were not re-signed or affected.

### Final suite, replays and limits

The final full serial CTest run passed **132/132** in **212.21 seconds** with `--repeat until-pass:3` ([complete log](proofs/continuation/ctest-verified.log)). Exactly one test needed a retry: the unchanged Qt GL compositor timed out once, then passed on its second attempt. Qt GL output passed on its first attempt in that run. The earlier full runs each passed 131/132 and retain their Qt GL output failure ([first](proofs/continuation/ctest-full-1.log), [second](proofs/continuation/ctest-final.log)); its separate fresh-HOME retry passed ([receipt](proofs/continuation/qt-gl-isolated-home.log)). No graphics test or acceptance assertion was changed.

All six RustDesk recordings and `appleads.mp4` passed eight-second native playback through `build/WAM.app`: **7/7**, with draw counts **19, 170, 191, 196, 195, 196, 222** in sorted recording order followed by appleads. Every run selected native, acknowledged a first frame, exited cleanly and reported accepted clock samples formatting to `1.0000` ([receipts](proofs/continuation/replays/results.json)). As before, clock rate is the requested transport rate, not an independent wall-clock measurement.

The required parallel builds succeeded. [Final identities](proofs/continuation/final-identities.json) record source and binary hashes; [frozen identities](proofs/continuation/frozen.json) remain equal to HEAD. No changes were staged or committed. Only this clone's launched child PIDs were controlled; no network operation or user audio-configuration change was performed.

**Numbered frozen-file proposals: none.** No item-1 recovery or package/SwiftUI implementation work remains deferred. Distribution validation remains blocked on a real Developer ID identity, trusted timestamping, notarization/stapling and same-team hardened-runtime playback. The package remains arm64/macOS 26 with this dependency build. Clean-machine/older-OS validation, additional architectures, WAM's own redistribution grant and the previously listed broader API roadmap remain release/roadmap deferrals. The app's shared-owner boundary is enforced; dynamic-framework app migration is still separate.
