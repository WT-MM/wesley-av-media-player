...
`libavutil-wamnative.61.dylib` and `libavformat-wamnative.63.dylib` from
Contents/Frameworks. Rebuild all three from the
matching source/configuration as one ABI closure; retain their @rpath install
names and LGPL-only configuration. Replace all three files in a copy of the app,
