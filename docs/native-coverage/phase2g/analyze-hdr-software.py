...
def managed(name):
 im=Image.open((Path('/private/tmp/wam-phase2g/captures/software') if name.endswith('h264') else r)/name/'display.png');return np.asarray(ImageCms.profileToProfile(im,ImageCms.ImageCmsProfile(io.BytesIO(im.info['icc_profile'])),ImageCms.createProfile('sRGB'),outputMode='RGB'),dtype=float)
rows=[]
for label in ['hlg','pq']:
 for suffix in (['']):
  names=[label+'-hevc'+suffix,label+'-h264'+suffix,label+'-hevc-wrong-matrix'+suffix,label+'-hevc-wrong-range'+suffix]
  if any(not (r/n/'display.png').exists() for n in names):continue
...
Path('/private/tmp/wam-phase2g/hdr-software-projections.json').write_text(json.dumps(rows,indent=2));print(json.dumps(rows,indent=2))
