...
def managed(name):
 im=Image.open(r/name/'display.png');return np.asarray(ImageCms.profileToProfile(im,ImageCms.ImageCmsProfile(io.BytesIO(im.info['icc_profile'])),ImageCms.createProfile('sRGB'),outputMode='RGB'),dtype=float)
rows=[]
for label in ['hlg','pq']:
 for suffix in (['','-ambient'] if label=='hlg' else ['']):
...
Path('/private/tmp/wam-phase2g/hdr-hardware-projections.json').write_text(json.dumps(rows,indent=2));print(json.dumps(rows,indent=2))
