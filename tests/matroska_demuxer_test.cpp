...
int main() {
  {
    constexpr std::array<std::uint8_t,52> avcc{0x01,0xf4,0x00,0x1e,0xff,0xe1,0x00,0x1e,0x67,0xf4,0x00,0x1e,0x91,0x9b,0x28,0x14,0x05,0xff,0x13,0x80,0xb5,0x01,0x01,0x01,0x40,0x00,0x00,0x03,0x00,0x40,0x00,0x00,0x0c,0x83,0xc5,0x8b,0x65,0x80,0x01,0x00,0x07,0x68,0xeb,0xe0,0x43,0x11,0x21,0x10,0xff,0xf8,0xf8,0x00};
    for (const auto range : {0U, 1U, 2U, 3U, 4U}) {
      FixtureSpec spec;
      spec.videoCodecPrivate = fromOctets(avcc);
      spec.videoPixelWidth = 640; spec.videoPixelHeight = 360;
      spec.videoRange = range;
      const auto prepared = prepareFixture(spec);
      expect(bool(prepared.outcome.asset) == (0 == 0 && range < 2),
             "444 requires limited coded range and unambiguous limited presentation range");
    }
  }
  {
    constexpr std::array<std::uint8_t,52> avcc{0x01,0xf4,0x00,0x1e,0xff,0xe1,0x00,0x1e,0x67,0xf4,0x00,0x1e,0x91,0x9b,0x28,0x14,0x05,0xff,0x13,0x80,0xb7,0x01,0x01,0x01,0x40,0x00,0x00,0x03,0x00,0x40,0x00,0x00,0x0c,0x83,0xc5,0x8b,0x65,0x80,0x01,0x00,0x07,0x68,0xeb,0xe0,0x43,0x11,0x21,0x10,0xff,0xf8,0xf8,0x00};
    for (const auto range : {0U, 1U, 2U, 3U, 4U}) {
      FixtureSpec spec;
      spec.videoCodecPrivate = fromOctets(avcc);
      spec.videoPixelWidth = 640; spec.videoPixelHeight = 360;
      spec.videoRange = range;
      const auto prepared = prepareFixture(spec);
      expect(bool(prepared.outcome.asset) == (1 == 0 && range < 2),
             "444 requires limited coded range and unambiguous limited presentation range");
    }
  }
  {
    constexpr std::array<std::uint8_t,51> fullRangeAvcc{0x01,0x64,0x00,0x1e,0xff,0xe1,0x00,0x1e,0x67,0x64,0x00,0x1e,0xac,0xd9,0x40,0xa0,0x2f,0xf9,0x70,0x16,0xe0,0x40,0x40,0xc8,0x00,0x00,0x03,0x00,0x08,0x00,0x00,0x03,0x01,0x90,0x78,0xb1,0x6c,0xb0,0x01,0x00,0x06,0x68,0xeb,0xe3,0xcb,0x22,0xc0,0xfd,0xf8,0xf8,0x00};
    for (const auto range : {0U, 1U, 2U}) {
      FixtureSpec spec;
      spec.videoCodecPrivate = fromOctets(fullRangeAvcc);
      spec.videoPixelWidth = 640; spec.videoPixelHeight = 360;
      spec.videoRange = range;
      const auto prepared = prepareFixture(spec);
      expect(prepared.outcome.asset &&
                 prepared.outcome.asset->descriptor()->tracks.front().video->fullRangeVideo == (range != 1),
             "unspecified Matroska range inherits the full-range VUI; explicit range overrides it");
    }
  }
  {
    FixtureSpec spec;
    spec.videoRange = 2;
    const auto prepared = prepareFixture(spec);
    expect(prepared.outcome.asset && prepared.outcome.asset->descriptor()->tracks.front().video->fullRangeVideo,
