...
for index in range(1, 16):
    steps.extend([f'new@120', f'load:{index}:{asset}@0'])
steps.append('report@2200')
for index in range(16):
    steps.extend([f'scroll:{index}:180:0:3@15', f'scroll:{index}:-180:0:3@15'])
steps.append('report@1000')
...
env.update(HOME=str(root / 'home'), WAM_NATIVE_BENCHMARK_TELEMETRY='1',
           ...
           WAM_TEST_QUIT_AFTER_MS='15000', WAM_PLAYBACK_METRICS_PATH=str(root / 'metrics.jsonl'),
...
with (root / 'log.txt').open('w') as log:
    ...
    for label, at in [('active', 3.8), ('closed', 11.0)]:
