...
candidate=sha(app);baseline={r['path']:r['status'] for r in json.loads((repo/'docs/native-coverage/phase2/corpus-results.json').read_text())}
rows=[]
for i,line in enumerate(pathlib.Path(a.corpus).read_text().splitlines()):
 ...
 status='NATIVE_OK' if rc==0 and 'native_selected' in text and 'first_frame_drawn' in text and not failures else 'REFUSED'
 rows.append(dict(path=str(asset),status=status,baseline=baseline.get(str(asset)),failures=failures,rc=rc,
