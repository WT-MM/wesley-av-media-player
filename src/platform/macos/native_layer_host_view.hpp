...
class NativeLayerHostView final {
 public:
  ...
  // Creates an unattached view; the caller owns window placement and geometry.
  static std::shared_ptr<NativeLayerHostView> createDetached(
      double width, double height, std::string* error = nullptr);
  [[nodiscard]] void* view() const noexcept;
