...
std::unique_ptr<NativeMediaSession> createQtNativeMediaSessionSystem(
    NativeMediaSessionSourceBinding binding,
    std::shared_ptr<void> externalLifetime,
    QtGlVideoItem* videoItem, std::string* error,
    std::shared_ptr<media::captions::LiveCaptionFeed> captionFeed) noexcept {
  if (!videoItem) {
    if (error) error->clear();
    assignError(error, "system native media session requires a Qt video item");
    return {};
  }
  return createNativeMediaSessionSystem(std::move(binding),
      std::move(externalLifetime), {videoItem, &createPresentation}, error,
      std::move(captionFeed));
}
}
